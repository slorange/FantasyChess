...
namespace FantasyChess.Game
{
    ...
    public class ChessView {//extends View implements View.OnTouchListener {
        ...
        public class Button{
            public int x, y, w, h;
            public string e;
            public Button(int x, int y, int w, int h, string e){
                this.x = x;
                this.y = y;
                this.w = w;
                this.h = h;
        ...
        private void drawBoard(Canvas canvas){
            ...
            foreach (PlayerMove m in board.valid) {
        ...
        private void drawCaptured(Canvas canvas){
            ...
            foreach (Piece p in board.p2captured){
            ...
            i = 0;
            foreach (Piece p in board.p1captured){
        ...
        long lastClickTime = DateTimeOffset.Now.ToUnixTimeMilliseconds();

       // @Override
        public bool onTouch(View v, Motione e) {
            long time = DateTimeOffset.Now.ToUnixTimeMilliseconds();
            if(time - lastClickTime < 150) return true;
            lastClickTime = time;
            ...
            foreach (Button bu in buttons){
                if(x >= bu.x && x <= bu.x+bu.w && y >= bu.y && y <= bu.y+bu.h){
                    HandleClick(bu.e);
                    return true;
                }
            }

            int a = (int)((e.getX() - minx) / squareSize);
            int b = (int)((e.getY() - miny) / squareSize);
        ...
        private void HandleClick(string e){
            if(e.Equals("gear")){
                Settings();
            }
            else if(e.Equals("Singleplayer")){
                SetMode(true);
                HideSettings();
            }
            else if(e.Equals("Multiplayer")){
                SetMode(false);
                HideSettings();
            }
            else if(e.Equals("Checkers") || e.Equals("Chess") || e.Equals("Fantasy Chess") || e.Equals("Test")){
                SetMode(e);
                HideSettings();
            }
        }
